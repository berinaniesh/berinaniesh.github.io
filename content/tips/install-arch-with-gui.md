...
```
#### Important
*When `genfstab` is run from the Fedora ISO, it creates an entry for zram swap as well, which doesn't work in Arch Linux out of the box (It needs manual setup). This will stop you from booting. So, make sure you remove the zram entry from the fstab file (`/mnt/etc/fstab`).*
...
```
Thank you [Mutahar](https://www.youtube.com/c/SomeOrdinaryGamers) for the name Billy. 
...
Hope you learnt something useful. If you have any feedback, send an [email](mailto://berinaniesh@gmail.com). You can also submit pull requests to the [github repo of this website](https://github.com/berinaniesh/berinaniesh.github.io).
